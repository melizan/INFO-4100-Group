...
load("~/Desktop/info/info4100_hita_data.rda" )
```
...
# Add cutoff timestamp to each row in your target dataset
completion_data <- completion_data %>%
  mutate(
...
con <- con %>%
  mutate(timestamp = as.numeric(as.POSIXct(created_at, tz = "UTC")))

feature_data <- completion_data %>%
  select(student_id, activity_id, cutoff_utc) %>%
  pmap_dfr(function(student_id, activity_id, cutoff_utc) {
    
    con_sub <- con %>%
      filter(student_id == student_id, timestamp < cutoff_utc)
    
    tibble(
      student_id = student_id,
      activity_id = activity_id,
      num_total_msgs = nrow(con_sub),
      num_student_msgs = sum(con_sub$system == 0),
      num_ai_msgs = sum(con_sub$system == 1),
      avg_student_msg_len = mean(con_sub$length_char[con_sub$system == 0], na.rm = TRUE),
      num_chat_conversations = n_distinct(con_sub$conversation_id[is.na(con_sub$activity_step_id)]),
      num_activity_msgs = sum(!is.na(con_sub$activity_step_id)),
      num_unique_activities_chatted = n_distinct(con_sub$activity_step_id[!is.na(con_sub$activity_step_id)])
    )
  })

final_data <- completion_data %>%
  left_join(feature_data, by = c("student_id", "activity_id"))
...
# add code here
model <- glm(incomplete ~ num_student_msgs + avg_student_msg_len + num_chat_conversations +
                               num_total_msgs + num_activity_msgs + num_unique_activities_chatted,
             data = train, family = "binomial")
# Now calculate TP/FP/FN again

# Get predictions
# add code here
pred_probs <- predict(model, train, type = "response")
pred_binary <- ifelse(pred_probs > 0.5, 1, 0)

true_pos  <- sum(pred_binary == 1 & train$incomplete == 1)
false_pos <- sum(pred_binary == 1 & train$incomplete == 0)
false_neg <- sum(pred_binary == 0 & train$incomplete == 1)
...
test_pred_binary <- ifelse(test_pred_probs > 0.5, 1, 0)
# Compute F1
true_pos_test  <- sum(test_pred_binary == 1 & test$incomplete == 1)
false_pos_test <- sum(test_pred_binary == 1 & test$incomplete == 0)
false_neg_test <- sum(test_pred_binary == 0 & test$incomplete == 1)
